<template>
  <div class="w-full">
    ...
    <div class="flex items-center justify-center mb-6">
      ...
      >
        <div
          class="rounded-full transition duration-500 ease-in-out h-8 w-8 flex items-center justify-center font-bold border-2"
          :class="
            step === 1
              ? 'border-indigo-600'
      ...
      <div
        class="flex-auto border-t-2 transition duration-500 ease-in-out mx-4"
      ...
      <div v-if="step === 1" class="space-y-6">
        <div>
          ...
          <input
            ...
            class="w-full border border-gray-300 rounded-xl px-5 py-3 shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
          />
        ...
        <div>
          ...
          <input
            ...
            class="w-full border border-gray-300 rounded-xl px-5 py-3 shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
          />
          <p v-if="errorConfirm" class="text-red-600 text-sm mt-1">
            {{ errorConfirm }}
          </p>
        </div>
      </div>
      ...
      >
        <div>
          <label
            for="hoLot"
            class="block text-sm font-semibold text-gray-700 mb-2"
            >Họ lót <span class="text-red-500">*</span></label
          >
          <input
            v-model="hoLot"
            type="text"
            ...
            class="w-full border border-gray-300 rounded-xl px-5 py-3 shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
          />
          <p v-if="errorHoLot" class="text-red-600 text-sm mt-1">
            {{ errorHoLot }}
          </p>
        </div>
        <div>
          ...
          <input
            ...
            class="w-full border border-gray-300 rounded-xl px-5 py-3 shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
          />
          <p v-if="errorTen" class="text-red-600 text-sm mt-1">
            {{ errorTen }}
          </p>
        </div>
      ...
      <div class="pt-6">
        <div v-if="step === 1" class="flex justify-end">
          <button
            @click="nextStep"
            type="button"
            class="w-full bg-indigo-600 text-white font-bold py-3 rounded-xl shadow-lg hover:bg-indigo-700 transition transform hover:-translate-y-1"
          >
            Tiếp tục
          </button>
        </div>
        <div v-if="step === 2" class="flex flex-col sm:flex-row gap-4">
          <button
            @click="prevStep"
            type="button"
            class="w-full sm:w-1/3 bg-slate-200 text-slate-700 font-bold py-3 rounded-xl hover:bg-slate-300 transition"
          >
            Quay lại
          </button>
          <button
            type="submit"
            class="w-full sm:w-2/3 bg-gray-800 text-white font-bold py-3 rounded-xl shadow-lg hover:bg-gray-700 transition transform hover:-translate-y-1"
          >
            Đăng ký tài khoản
          </button>
        </div>

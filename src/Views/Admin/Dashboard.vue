  ...
  <div>
    <div class="mb-8">
      ...
      <p class="mt-1 text-slate-500">
        Chào mừng trở lại, {{ userStore.username }}! Đây là tổng quan hệ thống
        của bạn.
      </p>
    </div>
    ...
    <div v-if="!loading && !error">
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div
          class="bg-white p-6 rounded-xl shadow-md flex items-center gap-4 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg"
        >
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div class="bg-white p-6 rounded-xl shadow-md">
          ...
          <div v-if="overdueList.length > 0" class="overflow-y-auto max-h-80">
            <table class="w-full text-sm text-left">
              <thead
                class="text-xs text-slate-600 uppercase bg-slate-50 sticky top-0"
              >
                <tr>
        ...
        <div class="bg-white p-6 rounded-xl shadow-md">
          <h2 class="text-xl font-bold text-slate-800 mb-4">
            Danh sách sách đã hết
          </h2>
          <div
            v-if="outOfStockBooks.length > 0"
